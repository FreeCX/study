...
void RK_OpenCL::opencl_prepare_kernel( vector *v0, unsigned int count_v0, double h, int n_x, int n_y, int n_z,
                                       vector_field field, unsigned int max_points) {

    _opencl_v0 = clCreateBuffer( _context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
                                 sizeof(vector) * count_v0, v0, NULL );
    _opencl_count_v0 = clCreateBuffer( _context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(int), 
                                       &count_v0, NULL );
    ...
    _opencl_field = clCreateBuffer( _context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
...
void RK_OpenCL::opencl_run_kernel( unsigned int count_v0, unsigned int max_points, Fiber **fibers ) {
    ...
    clFinish( _queue );
    if ( clEnqueueReadBuffer( _queue, _opencl_n_points, CL_TRUE, 0, sizeof(unsigned int) * count_v0,
                              n_points, 0, NULL, &_event ) == CL_INVALID_VALUE ) {
        printf("\nERROR: Failed to read buffer \"n_points\".\n");
        exit( -1 );
    }
    opencl_time( &_event );
    clReleaseEvent( _event );
    if ( clEnqueueReadBuffer( _queue, _opencl_points, CL_TRUE, 0, sizeof(vector) * count_v0 * max_points,
                              points, 0, NULL, &_event) == CL_INVALID_VALUE ) {
...
void RK_OpenCL::opencl_time( cl_event* timer ) {
    ...
    if ( clGetEventProfilingInfo( _event, CL_PROFILING_COMMAND_START, 
                                  (size_t)sizeof(cl_ulong), &_start, NULL ) != CL_SUCCESS ) {
        printf( "\nERROR: Failed to get profiling info.\n" );
    }
    if ( clGetEventProfilingInfo( _event, CL_PROFILING_COMMAND_END, 
                                  (size_t)sizeof(cl_ulong), &_finish, NULL ) != CL_SUCCESS ) {
        printf( "\nERROR: Failed to get profiling info.\n" );
    }
...
void RK_OpenCL::rk4_caller( FILE * std, size_t max_points, vector *v0, int count_v0, double h, 
                            int n_x, int n_y, int n_z, vector_field field, Fiber **fibers ) {
    opencl_init( (char*)"rk4_kernel", v0, count_v0, h, n_x, n_y, n_z, field, fibers, max_points );
    fprintf( std, "[ OpenCL] CPU/GPU time for RK4: %.4Es\n", _time * 1.0e-9 );
